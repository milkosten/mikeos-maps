...
class TripsCloudClient(
    ...
    /** A route from OSRM: an encoded polyline (precision 5), distance km, and ETA minutes. */
    data class Route(val polyline: String, val km: Double, val etaMin: Double)
    ...
    suspend fun route(
        ...
        try {
            client.newCall(unkeyed("/api/route").post(payload).build()).execute().use { resp ->
                ...
                Route(poly, o.optDouble("km", 0.0), o.optDouble("eta_min", 0.0))
            }
        } catch (e: Exception) {
